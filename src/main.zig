...
pub const ObjCopyOptions = struct {
    in_file: File,
    out_file: File,
    in_fmt: std.Target.ObjectFormat = .elf,
    out_fmt: std.Target.ObjectFormat = .elf,
    only_section: ?[]const u8 = null,
    pad_to: ?u64 = null,
    strip_debug: bool = false,
    strip_all: bool = false,
    only_keep_debug: bool = false,
    // TODO: use File instead of paths
    add_debuglink: ?[]const u8 = null,
    extract_to: ?[]const u8 = null,
};

pub fn objCopy(arena: Allocator, options: ObjCopyOptions) !void {
    const elf_hdr = try std.elf.Header.read(options.in_file);

    switch (options.out_fmt) {
        .hex, .raw => {
            if (options.strip_debug or options.strip_all or options.only_keep_debug)
                @panic("ELF to RAW or HEX copying does not support --strip");
            if (options.opt_extract != null)
                @panic("ELF to RAW or HEX copying does not support --extract-to");

            try emitElf(arena, options.in_file, options.out_file, options.elf_hdr, .{
                .ofmt = options.out_fmt,
        ...
        .elf => {
            if (elf_hdr.endian != @import("builtin").target.cpu.arch.endian())
                @panic("ELF to ELF copying only supports native endian");
            if (elf_hdr.phoff == 0) // no program header
                @panic("ELF to ELF copying only supports programs");
            if (options.only_section) |_|
                @panic("ELF to ELF copying does not support --only-section");
            if (options.pad_to) |_|
                @panic("ELF to ELF copying does not support --pad-to");

            try stripElf(arena, options.in_file, options.out_file, elf_hdr, .{
                .strip_debug = options.strip_debug,
                .strip_all = options.strip_all,
                .only_keep_debug = options.only_keep_debug,
                .add_debuglink = options.add_debuglink,
                .extract_to = options.extract_to,
            });
            return std.process.cleanExit();
        },
        else => {
            std.log.err("unsupported output object format: {s}", .{@tagName(options.out_fmt)});
            return error.UnsupportedOutputObjectFormat;
        },
    }
    return std.process.cleanExit();
}
...
fn emitElf(
    ...
    if (options.ofmt == .elf) {
        @panic("ELF to ELF copying is not implemented yet");
    }

    if (options.only_section) |target_name| {
        switch (options.ofmt) {
            .hex => @panic("hex format with sections is not implemented yet"),
            .raw => {
    ...
    switch (elf_hdr.is_64) {
        inline else => |is_64| {
            ...
            if (filter_complement) |flt| {
                ...
                const dbg_file = try std.fs.cwd().createFile(path, .{});
        ...
        pub fn parse(gpa: Allocator, in_file: File, header: elf.Header) !Self {
            ...
            const program_segments = blk: {
                if (@sizeOf(Elf_Phdr) != header.phentsize)
                    return error.UnexpectedPhentsize;
            ...
            const sections = blk: {
                if (@sizeOf(Elf_Shdr) != header.shentsize)
                    return error.UnexpectedShentsize;
        ...
        fn emit(self: *const Self, gpa: Allocator, out_file: File, in_file: File, options: EmitElfOptions) !void {
            ...
            const debuglink_name: u32 = blk: {
                ...
                if (self.raw_elf_header.e_shstrndx == elf.SHN_UNDEF)
                    @panic("no strtab, cannot add the debuglink section"); // TODO add the section if needed?
                ...
                for (self.sections[1..], sections_update[1..]) |section, update| {
                    ...
                    if (src.sh_offset != dest.sh_offset and section.segment != null and update.action != .empty and dest.sh_type != elf.SHT_NOTE) {
                        if (src.sh_offset > dest.sh_offset) {
                            ...
                            @panic("cannot adjust program segments");
                        }
                    }
...
const ElfFileHelper = struct {
    ...
    fn createDebugLink(path: []const u8) !DebugLink {
        ...
        const crc = try ElfFileHelper.computeFileCrc(file);
        return .{
